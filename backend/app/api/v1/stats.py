...
async def get_stats(
    ...
    active_clients_r = await db.execute(
        select(func.count(Client.id)).where(Client.status == ClientStatus.ACTIF)
    )
    active_clients = active_clients_r.scalar_one()

    prospects_r = await db.execute(
        select(func.count(Client.id)).where(Client.status == ClientStatus.PROSPECT)
    )
    ...
    active_contracts_r = await db.execute(
        select(func.count(Contract.id)).where(Contract.status == ContractStatus.ACTIF)
    )
    ...
    pending_r = await db.execute(
        select(func.count(Invoice.id)).where(Invoice.status == InvoiceStatus.EN_ATTENTE)
    )
    pending_invoices = pending_r.scalar_one()

    overdue_r = await db.execute(
        select(func.count(Invoice.id)).where(Invoice.status == InvoiceStatus.EN_RETARD)
    )
